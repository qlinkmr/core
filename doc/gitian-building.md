...
*Setup instructions for a Gitian build of Qlink Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Qlink
...
- [Building Qlink Core](#building-qlink-core)
...
Clone the git repositories for Qlink Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/qlinkmr
git clone https://github.com/qlinkpay/gitian.sigs.git
```
...
in the Qlink Core repository under 'Fetch and create inputs' to install sources which require
...
To build Qlink Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Qlink Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/qlink/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/qlinkmr
...
For example:
```bash
URL=https://github.com/crowning-/qlink.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit qlink=${COMMIT} --url qlink=${URL} ../qlink/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit qlink=${COMMIT} --url qlink=${URL} ../qlink/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit qlink=${COMMIT} --url qlink=${URL} ../qlink/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the qlink git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../qlink/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/qlink
SIGPATH=/some/root/path/qlink-detached-sigs

./bin/gbuild --url qlink=${BTCPATH},signature=${SIGPATH} ../qlink/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/qlink-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/qlink-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/qlink-osx-build.assert
```
...
[qlink/gitian.sigs](https://github.com/qlinkpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
