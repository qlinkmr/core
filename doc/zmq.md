...
The Qlink Core daemon can be configured to act as a trusted "border
router", implementing the qlink wire protocol and relay, making
...
The ZeroMQ feature in Qlink Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building qlinkd:
...
For instance:
    ...
    $ qlinkd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/qlinkd.tx.raw \
...
These options can also be provided in qlink.conf.
...
From the perspective of qlinkd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into qlinkd directly. Furthermore, no information is
...
using. Qlinkd appends an up-counting sequence number to each
