...
Currently, 10% of block rewards are set aside for the Qlink DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Qlink in a
credit pool. This credit pool mirrors the available credit supply in Qlink
Platform. When a certain amount of Qlink is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Qlink
Platform. Masternode identities receive the fees and can convert them back to
Qlink by withdrawing them from Qlink Platform.
...
- `bestCLHeightDiff` (`uint32`) is equal to the diff in heights from the mined
  block height
- `creditPoolBalance` (`int64`) is equal to all the qlink locked in all previous
  blocks minus all the qlink unlocked in all previous blocks plus all the block
...
### Removal of reject network messages from Qlink Core (BIP61)

This feature has been disabled by default since Qlink Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Qlink Core version 20.0.0 there are extra changes:
...
- It is possible to run Qlink Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `QL/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Qlink Core (BIP61)_](#removal-of-reject-network-messages-from-qlink-core-bip61) above for
...
- The `qlink-cli -getinfo` command now displays the wallet name and balance for
...
python test framework and `qlink-cli` tool both take advantage of this, so
for example:

```sh
qlink-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
qlink-cli -named createwallet mywallet load_on_startup=1
```
...
with Qlink’s product needs, such as SegWit and RBF, are excluded from our
...
- [v19.3.0](https://github.com/qlinkmr/blob/master/doc/release-notes/qlink/release-notes-19.3.0.md) released July/31/2023
- [v19.2.0](https://github.com/qlinkmr/blob/master/doc/release-notes/qlink/release-notes-19.2.0.md) released June/19/2023
- [v19.1.0](https://github.com/qlinkmr/blob/master/doc/release-notes/qlink/release-notes-19.1.0.md) released May/22/2023
...
[DIP-0023]: https://github.com/qlinkpay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/qlinkpay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/qlinkmr/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/qlinkmr/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/qlinkmr/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/qlinkmr/compare/v19.3.0...qlinkpay:v20.0.0
