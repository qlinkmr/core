...
impl PrivateKey {
    ...
    pub fn generate_qlink(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "qlink_helpers")]
    pub fn generate_qlink_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_qlink(rng))
            .collect()
    }
