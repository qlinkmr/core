...
const char * const BITCOIN_CONF_FILENAME = "qlink.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //qlink-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Qlink
    // macOS: ~/Library/Application Support/Qlink
    // Unix-like: ~/.qlink
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qlink";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Qlink";
#else
    // Unix-like
    return pathRet / ".qlink";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty qlink.conf if it does not exist
