...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsMenuAction->setStatusTip(tr("Send coins to a Qlink address"));
    ...
    coinJoinCoinsMenuAction->setStatusTip(tr("Send %1 funds to a Qlink address").arg(strCoinJoinName));
    coinJoinCoinsMenuAction->setToolTip(coinJoinCoinsMenuAction->statusTip());

    receiveCoinsMenuAction = new QAction(tr("&Receive"), this);
    receiveCoinsMenuAction->setStatusTip(tr("Request payments (generates QR codes and qlink: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Qlink addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Qlink addresses"));
    m_load_psbt_action = new QAction(tr("Load PSBT..."), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Qlink Transaction"));
    ...
    openAction->setStatusTip(tr("Open a qlink: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Qlink command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Qlink network", "", count));
    } else {
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the qlink logo
