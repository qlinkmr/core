...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 QL irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on qlinkcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Qlink features */
    { "qlink",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "qlink",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "qlink",               "gobject",                &gobject,                {} },
    { "qlink",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
