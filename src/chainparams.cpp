 ...
 * database.
 *
 * CBlock(hash=00000ffd590b14, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=e0028e, nTime=1709185283, nBits=1e0ffff0, nNonce=1404755, vtx=1)
...
{
    const char* pszTimestamp = "First MR Coin virtual reality";
    const CScript genesisOutputScript = CScript() << ParseHex("04f469324ddb89fcda57b81d0364b98036ac598792f5b2793f8cc8173485a22bf6aa66f679165167c5abbf23f04da37069ba63a70ed6b248960856890b6fd9ad4d") << OP_CHECKSIG;
...
public:
    CMainParams() {
        ...
        consensus.nSubsidyHalvingInterval = 949000; 
        ...
        consensus.defaultAssumeValid = uint256S("0x00000fb14091f7b9128028b83d82770de5df64166a8dae3a4abd1e9a7f31557d"); 
        ...
        genesis = CreateGenesisBlock(1709185283, 1404755, 0x1e0ffff0, 1, 10 * COIN);
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x00000fb14091f7b9128028b83d82770de5df64166a8dae3a4abd1e9a7f31557d"));
        assert(genesis.hashMerkleRoot == uint256S("0x52911b502a8d96c02c35eb5cf31150f095a59fd08eb7620afc45dc8a1a020c68"));
        ...
        vSeeds.emplace_back("node1.qlinkmr.com");
        vSeeds.emplace_back("node2.qlinkmr.com");
        ...
        // Qlink BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        // Qlink BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x88, 0xAD, 0xE4};

        // Qlink BIP44 coin type is '5'
        nExtCoinType = 5;
        ...
        vSporkAddresses = {"QinsJgVU5MBg9ZHYHmF85ZbpUM3BKShtXa"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x00000fb14091f7b9128028b83d82770de5df64166a8dae3a4abd1e9a7f31557d")},
            }
        };
        ...
        // getchaintxstats 17280 00000fb14091f7b9128028b83d82770de5df64166a8dae3a4abd1e9a7f31557d
        chainTxData = ChainTxData{
                1709185283, // * UNIX timestamp of last known number of transactions
...
public:
    CTestNetParams() {
        ...
        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x00000ecf617d347647054f5f366468654bda82d6d2f64565c8b08938405fa983"); 
        ...
        genesis = CreateGenesisBlock(1709185288UL, 1778585UL, 0x1e0ffff0, 1, 10 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x00000ecf617d347647054f5f366468654bda82d6d2f64565c8b08938405fa983"));
        assert(genesis.hashMerkleRoot == uint256S("0x52911b502a8d96c02c35eb5cf31150f095a59fd08eb7620afc45dc8a1a020c68"));
        ...
        // Testnet Qlink BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet Qlink BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet Qlink BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
        ...
        vSporkAddresses = {"LKuxwy6NsZsfGjCKFzaj49zK1KWGJsp6tH"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x00000ecf617d347647054f5f366468654bda82d6d2f64565c8b08938405fa983")},
            }
        };
        ...
        // getchaintxstats 17280 00000ecf617d347647054f5f366468654bda82d6d2f64565c8b08938405fa983
        chainTxData = ChainTxData{
                1709185288, // * UNIX timestamp of last known number of transactions
                0,    // * total number of transactions between genesis and that timestamp
                            //   (the tx=... number in the ChainStateFlushed debug.log lines)
                0,       // * estimated number of transactions per second after that timestamp
        };
...
public:
    explicit CDevNetParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock(1709185293, 5, 0x207fffff, 1, 10 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x09b1599d815009398ebb71a2344467a2edfa4b654310234656cf386deb4cbe31"));
        assert(genesis.hashMerkleRoot == uint256S("0x52911b502a8d96c02c35eb5cf31150f095a59fd08eb7620afc45dc8a1a020c68"));
        ...
        checkpointData = (CCheckpointData) {
            {
                { 0, uint256S("0x09b1599d815009398ebb71a2344467a2edfa4b654310234656cf386deb4cbe31")},
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        // privKey: 6vSuM4bzN9GPTPtGYVY4cWRgCWMCpdqvd66WPbGXg7DpYCnAwCP
        vSporkAddresses = {"LYFBfkbobSGtX4BRV7iRokQg3acFfkUW6M"};
        nMinSporkKeys = 1;
        ...
        // Regtest Qlink addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,48);
        // Regtest Qlink script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,49);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,176);
        // Regtest Qlink BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest Qlink BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest Qlink BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
